...
def get_analytic_derivatives_lih(g_num, states_num, complexsymmetric):
    ...
    for gi in range(g_num):
        ...
        pos_vec2 = np.loadtxt(f"coords.g{gi}.atom{2}.txt")

        mol.atom.extend([['H',(pos_vec0[0], pos_vec0[1], pos_vec0[2])]])
        mol.atom.extend([['H',(pos_vec1[0], pos_vec1[1], pos_vec1[2])]])
        mol.atom.extend([['H',(pos_vec2[0], pos_vec2[1], pos_vec2[2])]])
        ...
        for si in range(states_num):
            ...
            g0_s = uhf_to_ghf(g0_s_a, g0_s_b, 2, 1)
            g0_list.append(g0_s)

        a = np.loadtxt(f"noci.coeffs.g{gi}.txt")

        e1_noci = get_e1_noci(a, mol, 1, 2, g0_list, 3, complexsymmetric)
...
def get_numerical_derivatives_lih(g_num):
    ...
    for gi in range(30,g_num-1):
        ...
        delta_bl = (np.loadtxt(f"coords.g{gi}.atom0.txt")[2]
                    - np.loadtxt(f"coords.g{gi+1}.atom0.txt")[2])
...
# get_analytic_derivatives_lih(91, 2, False)

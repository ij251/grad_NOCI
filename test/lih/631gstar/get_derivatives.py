...
def get_analytic_derivatives_lih(g_num, states_num, complexsymmetric):
    ...
    for gi in range(g_num):
        ...
        pos_vec0 = np.loadtxt(f"coords.g{gi*5}.atom{0}.txt")
        pos_vec1 = np.loadtxt(f"coords.g{gi*5}.atom{1}.txt")
        ...
        for si in range(states_num):
            ...
            g0_s_a = np.loadtxt(f"mo.coeffs.g{gi*5}.s{si}.a.txt")
            g0_s_a_part = np.roll(g0_s_a[2:6], 1, axis=0)
            g0_s_a[2:6] = g0_s_a_part

            g0_s_b = np.loadtxt(f"mo.coeffs.g{gi*5}.s{si}.b.txt")
            g0_s_b_part = np.roll(g0_s_b[2:6], 1, axis=0)
            g0_s_b[2:6] = g0_s_b_part

            g0_s = uhf_to_ghf(g0_s_a, g0_s_b, 2, 2)
            g0_list.append(g0_s)

        a = np.loadtxt(f"noci.coeffs.g{gi*5}.txt")
...
def get_numerical_derivatives_lih(g_num, states_num):
...
get_analytic_derivatives_lih(90, 2, False)
# get_numerical_derivatives_lih(451, 2)
